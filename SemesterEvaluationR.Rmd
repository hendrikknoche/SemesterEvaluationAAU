...
numOfRowsEvalToImport<-89 #has to be exact !!!! rows on the evaluation tab in the excel file
semester<-1
TurnOffCourseEval<-TRUE
...
#show_semester2short <-FALSE; show_semester1 <-FALSE; 
showShort<-ifelse(TurnOffCourseEval, FALSE,TRUE)
ifelse(semester==1,show_semester1<-TRUE,show_semester2<-TRUE)
ifelse(show_semester2==TRUE & showShort,show_semester2short<-TRUE,show_semester2short<-FALSE)
...
Medialogy module grade distributions and evaluations in comparison to previous years (grey smoothed error bands indicating .95 confidence intervals of the percentage of students attaining that grade or providing that rating). 
...
```{r echo=FALSE,message=F, warning=F, out.width = '80%',eval=show_semester2short}
...
ggplot(dfem[dfem$sem==semester & dfem$item=="quality" & dfem$course=="ID",],aes(x=variable,y=percent))+
  theme_bw()+ #ggtitle("Interaction Design") +
